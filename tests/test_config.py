...
def test_merge():
    config = Config.from_files(["tests/configs/exp001.jsonnet", "tests/configs/patches/local.jsonnet"])
    assert config.train is not None
    assert config.train["hello"] == "world"

    config = Config.from_files(["tests/configs/exp001.jsonnet", "tests/configs/patches/server.jsonnet"])
